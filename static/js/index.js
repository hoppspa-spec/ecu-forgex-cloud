...
(() => {
  ...
  function renderPatches(ecu) {
    patchList.innerHTML = "";
    if (!ecu) {
      patchList.innerHTML = `<div class="patch">No hay parches disponibles</div>`;
      return;
    }
    ...
    patchList.querySelector("button").onclick = () => {
      updateYaml({
        status: "selected",
        id: "dpf_off",
        label: "DPF OFF",
      ...
      alert("Parche seleccionado (modo demo)");
    };
  }
  ...
  $("#binfile").addEventListener("change", async (e) => {
    ...
    lastCvn = crc32(buf).toString(16).toUpperCase();
  });
